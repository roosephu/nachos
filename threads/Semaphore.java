...
public class Semaphore {
    ...
    private static class PingTest implements Runnable {
        ...
        public void run() {
            for (int i = 0; i < 10; i++) {
                Lib.debug('S', "> ping.P " + Integer.toString(i));
                ping.P();
                Lib.debug('S', "> pong.V " + Integer.toString(i));
                pong.V();
                Lib.debug('S', "> >>><<< " + Integer.toString(i));
            }
        }
    ...
    public static void selfTest() {
        ...
        new KThread(new PingTest(ping, pong)).setName("semaphore-ping").fork();

        for (int i = 0; i < 10; i++) {
            Lib.debug('S', "> ping.V " + Integer.toString(i));
            ping.V();
            Lib.debug('S', "> pong.P " + Integer.toString(i));
            pong.P();
            Lib.debug('S', "> <<<>>> " + Integer.toString(i));
        }
    }
